...
def test_dict_to_backend_model():
    ...
    kwargs = dict_to_backend_model(tx, statement_id=1, tenant_id=2, user_id=3)
    ...
    assert kwargs["running_balance"] == Decimal("95.00")
    assert kwargs["date"] == date(2025, 1, 15)
...
def test_model_to_dict_roundtrip(tmp_path, monkeypatch):
    ...
    try:
        ...
        db.refresh(stmt)
        tx = Transaction(
            statement_id=stmt.id, tenant_id=1, user_id=1, date=date(2025, 1, 15),
