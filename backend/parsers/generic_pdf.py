...
from .parser_base import build_parse_result
...
class GenericPDFParser:
    ...
    # ------------------------------------------------------------------
    def extract_text(self, max_pages: int = 100) -> List[str]:
        pages_text: List[str] = []
        try:
            with pdfplumber.open(self.pdf_path) as pdf:
                page_count = len(pdf.pages)
                if page_count > max_pages:
                    print(f"[ERROR] PDF has {page_count} pages; maximum allowed is {max_pages}")
                    return []
        ...
        if len(total_text) < 200 and OCR_AVAILABLE:
            ocr_text = self._ocr_extract(max_pages=max_pages)
            if ocr_text:
                pages_text = [ocr_text]
    ...
    def _ocr_extract(self, max_pages: int = 100) -> str:
