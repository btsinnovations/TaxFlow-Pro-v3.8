"""Bank of America checking and credit card statement parser."""
...
def _parse_lines(text: str) -> List[Dict[str, Any]]:
    """Bank of America layout: MM/DD Description Withdrawal Deposit [Balance]."""
    ...
    for line in text.splitlines():
        ...
        if not line or any(s in line.lower() for s in ["subtotal", "total", "page", "statement period"]):
            continue
        # Match: MM/DD Description [Withdrawal] [Deposit] [Balance]
        m = re.match(
        ...
        # Parse amounts: could be withdrawal, deposit, balance or just amount + balance
        amounts = []
        ...
        is_debit = any(k in desc_upper for k in ["DEBIT", "WITHDRAWAL", "PAYMENT", "FEE", "CHARGE", "PURCHASE", "CHECK"])
        is_credit = any(k in desc_upper for k in ["DEPOSIT", "CREDIT", "REFUND", "REVERSAL"])

        if is_credit or (len(amounts) >= 2 and amounts[0] == 0):
            amount = abs(amounts[0]) if len(amounts) == 1 else abs(amounts[1]) if amounts[1] else abs(amounts[0])
            amount = abs(amount)
...
class BankOfAmericaParser:
    ...
    def can_handle(cls, text: str) -> bool:
        tl = text.lower()
        return "bank of america" in tl or "bofa" in tl
...
def parse_bank_of_america_pdf(pdf_path: str, raw_text: str) -> Dict[str, Any]:
    return BankOfAmericaParser.parse(pdf_path, raw_text)