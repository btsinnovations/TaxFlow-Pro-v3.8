"""Navy Federal Credit Union share draft / checking statement parser."""
...
def _parse_lines(text: str) -> List[Dict[str, Any]]:
    """Navy Federal: MM/DD Description Withdrawal Deposit [Running Balance]."""
    ...
    for line in text.splitlines():
        ...
        is_debit = any(k in desc_upper for k in ["WITHDRAWAL", "DEBIT", "PAYMENT", "FEE", "CHARGE", "PURCHASE"])
        is_credit = any(k in desc_upper for k in ["DEPOSIT", "CREDIT", "REFUND", "REVERSAL", "PAYROLL"])
        ...
        if key in seen:
            continue
        seen.add(key)
        transactions.append(make_tx(date_str, desc, amount, balance=balance))
    return transactions


class NavyFederalParser:
    institution_name = "Navy Federal"

    @classmethod
    def can_handle(cls, text: str) -> bool:
        tl = text.lower()
        return "navy federal" in tl or "navyfcu" in tl
...
def parse_navy_federal_pdf(pdf_path: str, raw_text: str) -> Dict[str, Any]:
    return NavyFederalParser.parse(pdf_path, raw_text)