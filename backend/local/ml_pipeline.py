...
def train_local_model(
    ...
    vectorizer_sha256 = _sha256_file(vectorizer_path)
...
def load_local_model_safe(model_dir: Optional[Path] = None) -> Optional[Pipeline]:
    ...
    if not meta_path.exists():
        import logging
        logging.warning("Local model exists but manifest is missing; refusing to load.")
        return None
    ...
    if expected_hash is None:
        import logging
        logging.warning("Local model manifest missing model_sha256; refusing to load.")
        return None

    actual_hash = _sha256_file(model_path)
    if actual_hash != expected_hash:
        raise TrainingError("Model integrity check failed")

    return joblib.load(model_path)


def load_local_model(model_dir: Optional[Path] = None) -> Optional[Pipeline]:
    """Load a previously trained local model, if it exists and passes integrity checks."""
    return load_local_model_safe(model_dir)
