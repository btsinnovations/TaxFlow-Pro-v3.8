...
echo -e "${YELLOW}=========================================="
echo -e "  TaxFlow Pro v3.9.2 — Starting Services"
echo -e "==========================================${NC}"
echo ""
...
echo -e "→ Starting backend server (localhost-only; set TAXFLOW_BIND_LAN=true to bind 0.0.0.0)..."
cd "$PROJECT_DIR"
...
"$VENV_DIR/bin/uvicorn" backend.api:app --reload --host 127.0.0.1 --port 8000 > "$LOG_FILE" 2>&1 &
BACKEND_PID=$!
