...
## 11. Internal Namespace Policy (Dependency Confusion Mitigation)

To prevent dependency-confusion attacks against TaxFlow Pro:

1. **Non-publishable project name.** `pyproject.toml` names the package `taxflow-pro-private`. This name is deliberately not registered on PyPI, so a malicious public package cannot shadow it.
2. **Internal packages use a reserved prefix.** Any locally-developed or private TaxFlow package must live under the `taxflow_private.*` Python namespace or the `@taxflow/` npm scope. These namespaces are not used by public registries, making confusion attempts easy to spot.
3. **Lockfile discipline.** `requirements-lock.txt` pins exact versions for every top-level dependency listed in `requirements.txt`. Regenerate and commit the lockfile whenever `requirements.txt` changes.
4. **Verification.** The test `backend/tests/test_dependency_confusion.py` enforces the three rules above. Run it in CI and before any release build.
...
- `backend/tests/test_dependency_confusion.py` — Dependency-confusion / namespace policy tests.
