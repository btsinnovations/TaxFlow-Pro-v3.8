...
import { getTaxRules } from '@/hooks/useAPI';
...
export default function TaxRules() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    getTaxRules((user as any)?.tenant_id)
      .then((data) => {
        if (cancelled) return;
        const normalized = (data || []).map((rule: any): TaxRule => ({
          id: rule.id || rule.rule_id || String(Math.random()),
          name: rule.name || 'Unnamed Rule',
          category: rule.category || 'Deductions',
          description: rule.description || '',
          appliesTo: Array.isArray(rule.applies_to) ? rule.applies_to : (rule.appliesTo || []),
          effectiveDate: rule.effective_date || rule.effectiveDate || '2026-01-01',
          threshold: rule.threshold,
          maximum: rule.maximum,
          overrideAllowed: Boolean(rule.override_allowed ?? rule.overrideAllowed),
          autoApply: Boolean(rule.auto_apply ?? rule.autoApply),
        }));
        setRules(normalized);
        if (normalized.length > 0 && !selectedRule) {
          setSelectedRule(normalized[0]);
        }
        setError(null);
      })
      .catch((err: any) => setError(err.message || 'Failed to load tax rules'))
      .finally(() => {
